...
export class AuthController {
  ...
  @ApiOperation({
    summary: 'Sign in',
    description:
      'The only login endpoint, for staff and customers alike. Returns a 15-minute access ' +
      'token and sets an httpOnly refresh cookie. The audience — staff or portal — is ' +
      'decided from the account and never from the request, and is reported in ' +
      '`audience` so the client knows which application to open. Invalid credentials and ' +
      'unknown accounts return the same generic 401 by design.',
  })
